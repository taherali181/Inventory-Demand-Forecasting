...
def recompute_alerts(
    ...
    Schedules an email notification (Change 11.7, see notifications.py) as
    a background task, but only when at least one alert was genuinely
    newly opened here — never on a recompute that only refreshes existing
    alerts' current_value or resolves some, and never when
    settings.alert_notification_emails is empty (notifications.py's own
    early-return also guards this, but skipping the schedule call entirely
    keeps a no-op recompute from adding background-task overhead at all).
    ...
    if newly_created_alerts and settings.alert_notification_emails:
        background_tasks.add_task(send_new_alert_notifications, [a.id for a in newly_created_alerts])
