...
def adjust_stock(
    ...
    # stock_level_lock (in-process) + get_or_create_stock_level's
    # with_for_update() (DB-level, real on Postgres) together guard the
    # read-modify-write below — see stock_ops.py for why both are needed.
    # Held through db.commit(), not just the increment, since that's where
    # the write actually lands.
    with stock_level_lock(payload.product_id, payload.warehouse_id):
        stock_level = get_or_create_stock_level(db, payload.product_id, payload.warehouse_id)
